  ...
  <div class="flex-1 overflow-auto pb-20 md:pb-0 bg-slate-50">
    ...
    <div class="p-4 max-w-2xl mx-auto space-y-2.5">
      ...
      <div v-for="s in sorted" :key="s.id"
        class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex gap-3 hover:shadow-md hover:border-primary-200 transition-all cursor-pointer"
        @click="openSession(s)">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0">
              <RouterLink :to="`/patients/${s.patientId}`" @click.stop class="font-semibold text-primary-600 hover:text-primary-700 truncate block transition-colors">
                {{ patientName(s.patientId) }}
              </RouterLink>
              <p class="text-xs text-slate-400 mt-0.5">{{ formatDateES(s.date) }} · {{ s.duration }} min</p>
            </div>
            <IconButton variant="danger" @click.stop="deleteSession(s.id)" class="shrink-0">
...
function openSession(s: Session) {
  router.push(`/patients/${s.patientId}?tab=sessions&session=${s.id}`)
}
