<template>
  ...
  <div v-else class="flex h-screen overflow-hidden bg-slate-50">
    ...
    <aside class="hidden md:flex flex-col w-64 bg-slate-900 shrink-0">
      ...
      <nav class="flex-1 px-3 py-4 space-y-0.5">
        <RouterLink v-for="item in navItems" :key="item.to" :to="item.to"
          class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-slate-400 hover:bg-slate-800 hover:text-white transition-all text-sm font-medium"
          activeClass="bg-primary-600 text-white shadow-sm hover:bg-primary-600">
      ...
      <div class="px-3 py-4 border-t border-slate-800 space-y-0.5">
        <RouterLink to="/settings"
          class="flex items-center gap-3 w-full px-4 py-2.5 rounded-xl text-slate-400 hover:bg-slate-800 hover:text-white transition-all text-sm font-medium"
        ...
        <button @click="handleSignOut"
          class="flex items-center gap-3 w-full px-4 py-2.5 rounded-xl text-slate-400 hover:bg-slate-800 hover:text-red-400 transition-all text-sm font-medium">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 shrink-0">
    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex z-50 shadow-[0_-4px_12px_rgba(0,0,0,0.06)]">
      <RouterLink v-for="item in navItems" :key="item.to" :to="item.to"
        class="flex-1 flex flex-col items-center gap-0.5 py-2.5 text-slate-400 text-xs font-medium transition-colors"
...
const ExercisesIcon = () => h('svg', { xmlns: 'http://www.w3.org/2000/svg', fill: 'none', viewBox: '0 0 24 24', 'stroke-width': '1.5', stroke: 'currentColor' }, [
...
const navItems = [
  ...
  { to: '/exercises', label: 'Ejercicios', icon: ExercisesIcon },
]
</script>
